...
function StatusIcon({ status }: { status?: "ok" | "error" | "testing" }) {
      ...
      {connections.map((conn) => (
        <Card key={conn.id} className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            <StatusIcon status={testResults[conn.id]} />
            <div>
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" aria-label="Test connection" onClick={() => onTest(conn.id)}>
              <Play className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="icon" aria-label="Edit connection" onClick={() => onEdit(conn)}>
              <Pencil className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="icon" aria-label="Delete connection" onClick={() => onDelete(conn.id)}>
