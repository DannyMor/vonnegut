...
def test_database_adapter_defines_interface():
    ...
    assert hasattr(DatabaseAdapter, "fetch_tables")
    assert hasattr(DatabaseAdapter, "fetch_schema")
    assert hasattr(DatabaseAdapter, "fetch_sample")
```
...
class DatabaseAdapter(ABC):
    ...
    async def fetch_schema(self, table: str) -> list[dict]:
        """Returns list of {column, type, nullable, is_primary_key}."""
        pass

    @abstractmethod
    async def fetch_sample(self, table: str, rows: int = 10) -> list[dict]:
        pass
```
...
### Task 9: Postgres Direct Adapter

**Files:**
- Create: `backend/src/vonnegut/adapters/postgres_direct.py`
- Create: `backend/tests/test_adapter_postgres_direct.py`

Tests use a mock to avoid needing a real Postgres instance.

- [ ] **Step 1: Write the failing tests**

```python
# backend/tests/test_adapter_postgres_direct.py
import pytest
...
@pytest.mark.asyncio
async def test_connect_creates_connection(adapter):
    with patch("vonnegut.adapters.postgres_direct.AsyncConnection.connect", new_callable=AsyncMock) as mock_connect:
...
async def test_fetch_tables(adapter):
    ...
    mock_cursor.fetchall = AsyncMock(return_value=[("users",), ("orders",)])
...
async def test_fetch_schema(adapter):
    ...
    assert schema[1]["is_primary_key"] is False
...
async def test_fetch_sample(adapter):
    ...
    # Fix: description[i].name doesn't work with MagicMock, use column index approach
...
```bash
git add backend/
git commit -m "feat: add Postgres direct adapter with psycopg3"
```
...
async def test_create_connection(manager):
    with patch("vonnegut.services.connection_manager.encrypt_config", side_effect=lambda c, k: c):
...
async def test_get_connection(manager):
    ...
    with patch("vonnegut.services.connection_manager.decrypt_config", side_effect=lambda c, k: c):
...
async def test_list_tables(client):
    ...
    mock_adapter.fetch_tables = AsyncMock(return_value=["users", "orders"])
    mock_adapter.disconnect = AsyncMock()

    with patch("vonnegut.routers.explorer.get_adapter", return_value=mock_adapter):
        resp = await client.get(f"/api/v1/connections/{conn_id}/tables")
    assert resp.status_code == 200
    assert resp.json() == ["users", "orders"]
...
async def test_get_table_schema(client):
    conn_id = await _create_connection(client)
    mock_adapter = AsyncMock()
    mock_adapter.fetch_schema = AsyncMock(return_value=[
        {"column": "id", "type": "integer", "nullable": False, "is_primary_key": True},
        {"column": "name", "type": "text", "nullable": True, "is_primary_key": False},
    ])
    mock_adapter.disconnect = AsyncMock()
...
async def test_get_table_sample(client):
    conn_id = await _create_connection(client)
    mock_adapter = AsyncMock()
    mock_adapter.fetch_sample = AsyncMock(return_value=[
        {"id": 1, "name": "Alice"},
        {"id": 2, "name": "Bob"},
    ])
    mock_adapter.disconnect = AsyncMock()
...
- [ ] **Step 3: Write the adapter factory helper**

```python
# Add to backend/src/vonnegut/adapters/base.py (append after DatabaseAdapter class)

async def get_adapter(connection: dict) -> DatabaseAdapter:
    """Factory: creates and connects the right adapter for a connection."""
...
from vonnegut.adapters.base import get_adapter
...
async def list_tables(conn_id: str, request: Request):
    ...
    if conn is None:
        raise HTTPException(status_code=404, detail="Connection not found")
    adapter = await get_adapter(conn)
    try:
...
async def test_connection_test_success(client):
    ...
    with patch("vonnegut.routers.connections.get_adapter", return_value=mock_adapter):
...
@pytest.mark.asyncio
async def test_connection_test_failure(client):
    create_resp = await client.post("/api/v1/connections", json={
    ...
    with patch("vonnegut.routers.connections.get_adapter", side_effect=Exception("Connection refused")):
        resp = await client.post(f"/api/v1/connections/{conn_id}/test")
    assert resp.status_code == 200
...
@pytest.mark.asyncio
async def test_run_test_migration(runner):
    source_adapter = AsyncMock()
    source_adapter.fetch_sample = AsyncMock(return_value=[
        {"id": 1, "name": "alice"},
        {"id": 2, "name": "bob"},
    ])
    ...
    result = await runner.run_test(source_adapter, transformations, rows=10)
...
@pytest.mark.asyncio
async def test_run_migration_basic(runner):
    source_adapter = AsyncMock()
    source_adapter.execute = AsyncMock(return_value=[{"count": 5}])
    source_adapter.fetch_sample = AsyncMock(return_value=[
        {"id": 1, "name": "alice"},
        {"id": 2, "name": "bob"},
    ])
    ...
    target_adapter._conn = AsyncMock()
    target_adapter.execute = AsyncMock(return_value=[])
    ...
    result = await runner.run(
        ...
        cancel_flag=MagicMock(is_set=MagicMock(return_value=False)),
    )
    assert result["status"] == "completed"


@pytest.mark.asyncio
async def test_run_migration_exceeds_row_limit(runner):
    source_adapter = AsyncMock()
    source_adapter.execute = AsyncMock(return_value=[{"count": 200_000}])
...
class MigrationRunner:
    ...
    async def run_test(
        ...
        before = await source_adapter.fetch_sample("", rows=rows) if hasattr(source_adapter.fetch_sample, '__call__') else []
        # For test mode, fetch_sample is called directly on the mock
        before = await source_adapter.fetch_sample(rows=rows)
...
async def test_migration(mig_id: str, request: Request):
    ...
    source_adapter = await get_adapter(source_conn)

    try:
        engine = TransformationEngine()
        runner = MigrationRunner(engine=engine)
        before = await source_adapter.fetch_sample(row["source_table"], rows=10)
        after = engine.apply_pipeline(list(before), transform_dicts)
        return {"before": before, "after": after}
    finally:
...
async def run_migration(mig_id: str, request: Request):
    ...
    async def _run():
        ...
        source_adapter = await get_adapter(source_conn)
        target_adapter = await get_adapter(target_conn)
